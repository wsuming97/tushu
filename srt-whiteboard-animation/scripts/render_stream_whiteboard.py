...
def _distance_weighted_indices(
    samples: list[tuple[int, int]],
    target_frames: int,
    pen_lifts: set[int] | None = None,
) -> list[int]:
    """
    按笔迹的实际像素距离（而非均匀索引）分配帧。

    原理：
    - 计算所有相邻采样点之间的欧氏距离（抬笔处距离记为 0）
    - 将总帧数按累积距离比例分配给每段笔画
    - 视觉笔速恒定：长段笔画分配更多帧，短段分配更少帧

    解决的问题：
    - 消除骨架追踪中「短起始笔画占用过多帧导致视觉卡顿」
    - 消除「大量采样点聚集在小区域导致笔尖原地停滞」
    """
    n = len(samples)
    if n == 0 or target_frames <= 0:
        return []
    if n == 1 or target_frames == 1:
        return [n - 1] * max(1, target_frames)
    if pen_lifts is None:
        pen_lifts = set()

    # 1. 计算逐段像素距离（抬笔处距离记为 0，不分配帧给跳跃）
    seg_dists = [0.0]  # seg_dists[i] = samples[i-1] 到 samples[i] 的距离
    for i in range(1, n):
        if i in pen_lifts:
            seg_dists.append(0.0)  # 抬笔跳跃不计距离
        else:
    ...
    # 2. 累积距离
    cum_dist = [0.0]
    for d in seg_dists[1:]:
        cum_dist.append(cum_dist[-1] + d)
    total_dist = cum_dist[-1]

    # 如果总距离极小（所有点几乎重叠），退回均匀映射
    if total_dist < 1.0:
        return _frame_progress_indices(n, target_frames)

    # 3. 每帧对应一个目标距离里程碑，用二分查找映射到采样索引
    import bisect
    indices: list[int] = []
    for f in range(target_frames):
        target_d = f * total_dist / (target_frames - 1)
...
class RegionStreamRenderer:
    ...
    def _lay_ink(self, writer, frames: int, samples: list[tuple[int, int]],
        ...
        if n == 0:
            ...
            return
        # 距离加权分配：长笔画多帧，短笔画少帧，笔速视觉匀速
        idx_for_frame = _distance_weighted_indices(samples, frames, pen_lifts)
        last: int | None = None
